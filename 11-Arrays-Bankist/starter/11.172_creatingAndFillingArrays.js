...
arr = new Array(3);
console.log(arr); // Creates an array with 3 empty values

console.log(arr.map(() => 5)); // Methods on empty array doesn't work except below

console.log(arr.fill(5));

// fill method with another parameter
arr = new Array(8);
console.log(arr.fill(3, 2, 5)); // Fills 3 from index 2 and fills empty again from index 4: [empty × 2, 3, 3, 3, empty × 3]

// fill method on existing array
arr = [1, 2, 3, 4, 5, 6, 7];
console.log(arr.fill(23, 1, 5));

// 2) Create an array with all same values using FROM method on Array constructor/object
console.log(Array.from({ length: 7 }, () => 3));

// Create an array with a sequence of values using from method
console.log(Array.from({ length: 7 }, (cur, i) => i + 1));
console.log(Array.from({ length: 7 }, (_, i) => i + 1)); // replace cur parameter with a throwaway variable as we are not using cur param in arrow function

// 3) Read movements from UI and calculate sum unlike reading data from backend
// Code begins at 283....
